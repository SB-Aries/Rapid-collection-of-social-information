...
class WebScraperGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("社工信息快速收集工具- GUI")
    ...
    def start_scraping(self):
        ...
        if not self.email_var.get() and not self.phone_var.get():
            messagebox.warning("提示", "请至少选择一项爬取内容（邮箱/手机号）")
            return
    ...
    def save_results(self):
        """保存结果到文件夹（仅保存唯一的邮箱和手机号）"""
        if not self.all_email_results and not self.all_phone_results:
...
if __name__ == "__main__":
    ...
    try:
        ...
        root_temp = tk.Tk()
        root_temp.withdraw()
        messagebox.showinfo("提示", "未检测到requests库，正在自动安装...")
        root_temp.destroy()
